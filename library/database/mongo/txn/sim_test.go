...
import (
	...
	"github.com/lzw5399/go-common-public/library/database/mongo/dbtest"
	"github.com/lzw5399/go-common-public/library/database/mongo/txn"
)
	...
	for i := 0; i < params.workers; i++ {
		go func() {
			...
			for {
				...
				var oldExists bool
				if params.reinsertCopy || params.reinsertZeroed {
					if err := accounts.FindId(change.origin).One(&old); err != mongo.ErrNotFound {
						c.Check(err, IsNil)
