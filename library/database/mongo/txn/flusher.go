...
import (
	...
	"github.com/lzw5399/go-common-public/library/database/mongo/bson"
)
...
func (f *flusher) rescan(t *transaction, force bool) (revnos []int64, err error) {
	...
	RetryDoc:
		retry++
		c := f.tc.Database.C(dkey.C)
		if err := c.FindId(dkey.Id).Select(txnFields).One(&info); err == mongo.ErrNotFound {
			// Document is missing. Look in stash.
			chaos("")
			if err := f.sc.FindId(dkey).One(&info); err == mongo.ErrNotFound {
...
func (f *flusher) assert(t *transaction, revnos []int64, pull map[bson.ObjectId]*transaction) error {
	...
	for i, op := range t.Ops {
		...
		if err := c.Find(qdoc).Select(bson.D{{"_id", 1}}).One(nil); err == mongo.ErrNotFound {
...
func (f *flusher) abortOrReload(t *transaction, revnos []int64, pull map[bson.ObjectId]*transaction) (err error) {
	...
	chaos("set-aborted")
	if err := f.tc.UpdateId(t.Id, udoc); err != nil && err != mongo.ErrNotFound {
		return err
	}
	t.State = taborted
...
func (f *flusher) apply(t *transaction, pull map[bson.ObjectId]*transaction) error {
	...
	for i := range t.Ops {
		...
		if err == nil {
			outcome = "DONE"
		} else if err == mongo.ErrNotFound || mongo.IsDup(err) {
			outcome = "MISS"
			err = nil
		} else {
			outcome = err.Error()
