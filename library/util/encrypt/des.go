...
//加密
func DesCBCEncrypt(src []byte, key []byte, iv []byte) []byte {
	...
	// 明文组数据填充
	paddingText := PKCS7Padding(src, block.BlockSize())
...
// 解密：
func DesCBCDecrypt(src []byte, key []byte, iv []byte) []byte {
	// 创建并返回一个使用DES算法的cipher.Block接口
	block, err := des.NewCipher(key)
	if err != nil {
		panic(err)
...
func DecryptDES_ECB_SAFE(src, key string) (string, error) {
...
//ECB解密
func DecryptDES_ECB_V2(src, key string) (result string, err error) {
	defer func() {
